...
|---|---|---|---|
| CODE_REVIEW L4 | implemented | `src/adapter/tdx/client.py` | TDX heartbeat now uses `get_logger` instead of `print`. |
...
| CODE_SMELL_REVIEW D2.5 | already closed | `src/adapter/qmt/client.py` | `subscribe_quote` reuses an existing queue when present. |
...
| CODE_SMELL_REVIEW C2.2 | implemented | `src/adapter/qmt/client.py` | Removed dead `list_type` branch and clarified QMT returns codes only. |
...
| CODE_SMELL_REVIEW U2.2 | implemented | `src/adapter/tdx/client.py` | Heartbeat logging now uses logger. |
| CODE_SMELL_REVIEW U2.4 | already closed | route helpers | Old `_get_adapter` helper is gone; v1 uses `_get_provider`. |
| CODE_SMELL_REVIEW O2.3 | implemented | `src/adapter/qmt/client.py` | Deleted dead `list_type` branch. |
...
- `UV_CACHE_DIR=.uv-cache uv run pytest tests/unit/test_repository_hygiene.py -q`
  passed: 17 tests.
- `UV_CACHE_DIR=.uv-cache uv run pytest tests/unit/test_repository_hygiene.py tests/unit/test_tdx_bridge.py tests/unit/test_tdx_runtime.py tests/unit/test_tdx_normalization.py -q`
  passed: 50 tests.
