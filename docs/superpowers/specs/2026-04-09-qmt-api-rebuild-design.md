...
|------|------|------|-----------------|
| GET | `/stock-list-in-sector` | `sector: str` | `get_stock_list_in_sector(sector_name)` |
...
**板块管理**: `get_sector_list`, `download_sector_data`, `get_index_weight`, `download_index_weight`, `create_sector_folder`, `create_sector` (QMT签名), `add_sector`, `remove_stock_from_sector`, `remove_sector`, `reset_sector`

**ETF/可转债**: `get_cb_info`, `download_cb_data`, `get_ipo_info` (QMT签名), `get_etf_info`, `download_etf_info`
...
- `order_stock_async(...)` → `int`
- `cancel_order_stock_async(order_id)` → `int`
- `query_stock_order(order_id)` → `dict`
- `query_stock_trades()` → `list[dict]`
- `query_stock_position(stock_code)` → `dict`
