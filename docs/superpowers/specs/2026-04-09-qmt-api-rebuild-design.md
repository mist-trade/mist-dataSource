...
5. **签名冲突统一 base.py** — 直接修改 base.py 中签名不兼容的方法，使其同时适配 TDX 和 QMT，各 adapter 内部做参数映射
...
**板块管理**: `get_sector_list`, `download_sector_data`, `get_index_weight`, `download_index_weight`, `create_sector_folder(parent_node, folder_name, overwrite)`, `create_sector(parent_node, sector_name, overwrite)` (直接使用base.py同名方法，base.py签名已改为`**kwargs`), `add_sector`, `remove_stock_from_sector`, `remove_sector`, `reset_sector`

**ETF/可转债**: `get_cb_info`, `download_cb_data`, `get_ipo_info(start_time, end_time)` (直接使用base.py同名方法，base.py签名已改为`**kwargs`), `get_etf_info`, `download_etf_info`
...
**需要修改签名的方法**（让 TDX 和 QMT 都能用）：

| 方法 | 当前签名 | 修改后签名 | 说明 |
|------|---------|-----------|------|
| `create_sector` | `(self, name: str, stocks: list[str])` | `(self, **kwargs: Any)` | TDX 用 `name/stocks`，QMT 用 `parent_node/sector_name/overwrite`，各自路由传对应参数 |
| `get_ipo_info` | `(self, ipo_type: int = 0, ipo_date: int = 0)` | `(self, **kwargs: Any)` | TDX 用 `ipo_type/ipo_date`，QMT 用 `start_time/end_time`，各自路由传对应参数 |

TDX adapter 内部实现改为从 kwargs 取参数，TDX 路由调用方式不变。
