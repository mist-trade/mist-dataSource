...
5. **签名冲突用新方法名解决** — 当 QMT SDK 签名与 base.py 已有方法不兼容时，在 base.py 新增 QMT 专属方法名，不修改 TDX 已有签名
...
| POST | `/create-sector` | body: `{parent_node, sector_name, overwrite}` | `create_sector_node(...)` |
...
| GET | `/ipo-info` | `start_time, end_time` | `get_ipo_info_by_date(start_time, end_time)` |
...
**板块管理**: `get_sector_list`, `download_sector_data`, `get_index_weight`, `download_index_weight`, `create_sector_folder(parent_node, folder_name, overwrite)`, `create_sector_node(parent_node, sector_name, overwrite)` (QMT专属名，避免与base.py的`create_sector(name, stocks)`冲突), `add_sector`, `remove_stock_from_sector`, `remove_sector`, `reset_sector`

**ETF/可转债**: `get_cb_info`, `download_cb_data`, `get_ipo_info_by_date(start_time, end_time)` (QMT专属名，避免与base.py的`get_ipo_info(ipo_type, ipo_date)`冲突), `get_etf_info`, `download_etf_info`
...
- `create_sector_node(parent_node, sector_name, overwrite)` → `str` — 与已有 `create_sector(name, stocks)` 区分
...
- `get_ipo_info_by_date(start_time, end_time)` → `list[dict]` — 与已有 `get_ipo_info(ipo_type, ipo_date)` 区分
- `get_etf_info()` → `dict`
...
以下 QMT SDK 方法签名与 base.py 已有方法不兼容，通过新增 base.py 方法名解决：

| base.py 已有方法 | QMT SDK 实际签名 | 新增 base.py 方法名 | 路由端点 |
|---|---|---|---|
| `create_sector(name, stocks)` | `create_sector(parent_node, sector_name, overwrite)` | `create_sector_node(parent_node, sector_name, overwrite)` | `POST /create-sector` |
| `get_ipo_info(ipo_type, ipo_date)` | `get_ipo_info(start_time, end_time)` | `get_ipo_info_by_date(start_time, end_time)` | `GET /ipo-info` |
| `delete_sector(name)` | `remove_sector(sector_name)` | `remove_sector(sector_name)` (已有) | `POST /remove-sector` |
