...
```python
generate_index_data(formula_name, formula_param = {}, stock_list = [], period = '1d', dividend_type = 'none', start_time = '', end_time = '', fill_mode = 'fixed', fill_value = float('nan'), result_path = '')
```
...
- 返回
  - None
- 备注
  - 必须连接投研端使用，传入的formula_name需要存在于投研端中
...
's_fa_eps_basic'                            #基本每股收益
's_fa_eps_diluted'                          #稀释每股收益
's_fa_roe_weighted'                         #加权平均净资产收益率
's_fa_roe_ttm'                              #净资产收益率(ttm)
's_fa_roe'                                  #净资产收益率
's_fa_bps'                                  #每股净资产
's_fa_cfps'                                 #每股经营现金流
's_fa_op'                                   #每股营业收入
...
'm_anntime'                                 #披露日期
'm_timetag'                                 #截止日期
'shareholder_num'                           #股东户数
'avg_hold_num'                              #平均持股数
'avg_hold_ratio'                            #平均持股比例
```
...
xttrader是xtquant库中提供交易相关功能的模块，本模块旨在提供完整的交易功能，支持股票、期货等品种的交易操作。
...
- 2024-05-24
  - 添加通用数据导出接口`export_data`
  - 添加通用数据查询接口`query_data`
- 2024-06-27
  - 添加外部成交导入接口`sync_transaction_from_external`
...
xttrader提供和MiniQmt的交易交互接口，本质是和MiniQmt建立连接，由MiniQmt处理交易请求，再把结果回传返回到python层。使用的交易账号以及能执行的交易操作和MiniQmt是一致的，要检查账号状态或者切换账号时直接操作MiniQmt即可。
...
| 代码 | 名称 |
|------|------|
| xtconstant.STOCK_BUY | 股票买入 |
| xtconstant.STOCK_SELL | 股票卖出 |
| xtconstant.FINANCE_BUY | 融资买入 |
| xtconstant.LOAN_SELL | 融券卖出 |
| xtconstant.BUY_COVER | 买券还券 |
| xtconstant.SELL_REPAY | 卖券还款 |
| xtconstant.CASH_REPAY | 现金还款 |
| xtconstant.STOCK_REPAY | 现券还券 |
| xtconstant.ETF_PURCHASE | ETF申购 |
| xtconstant.ETF_REDEMPTION | ETF赎回 |
...
| 代码 | 名称 |
|------|------|
| xtconstant.FUND_TRANSFER_IN | 银行转证券 |
| xtconstant.FUND_TRANSFER_OUT | 证券转银行 |

### 多空方向(direction)

| 代码 | 名称 |
|------|------|
| xtconstant.DIRECTION_LONG | 多头 |
| xtconstant.DIRECTION_SHORT | 空头 |

### 交易操作(offset_flag)

| 代码 | 名称 |
|------|------|
| xtconstant.OFFSET_OPEN | 开仓 |
| xtconstant.OFFSET_CLOSE | 平仓 |
| xtconstant.OFFSET_CLOSETODAY | 平今 |
| xtconstant.OFFSET_CLOSEYESTERDAY | 平昨 |
...
class XtOrder:
    def __init__(self):
        ...
        self.order_type = 0  # 委托类型
        self.price = 0.0  # 委托价格
        self.volume = 0  # 委托数量
        self.traded_volume = 0  # 成交数量
        self.status = 0  # 委托状态
        self.order_time = ""  # 委托时间
        self.trade_time = ""  # 成交时间
        self.direction = 0  # 多空
        self.offset_flag = 0  # 交易操作
```
...
class XtPosition:
    def __init__(self):
        ...
        self.stock_name = ""  # 股票名称
        self.volume = 0  # 持仓数量
        self.available = 0  # 可用数量
        self.open_price = 0.0  # 开仓价
        self.avg_price = 0.0  # 成本价
        self.current_price = 0.0  # 当前价
        self.market_value = 0.0  # 市值
        self.profit = 0.0  # 盈亏
        self.profit_ratio = 0.0  # 盈亏比例
        self.direction = 0  # 多空
```
...
```python
class XtPositionStatistics:
    def __init__(self):
        self.account_id = ""  # 账号
        self.total_long_volume = 0  # 总多头持仓
        self.total_short_volume = 0  # 总空头持仓
        self.total_long_profit = 0.0  # 总多头盈亏
        self.total_short_profit = 0.0  # 总空头盈亏
```
...
class XtCreditDetail:
    def __init__(self):
        ...
        self.total_profit = 0.0  # 总盈亏
        self.margin = 0.0  # 保证金
        self.credit_line = 0.0  # 信用额度
        self.used_credit_line = 0.0  # 已用信用额度
```
...
```python
class CreditSubjects:
    def __init__(self):
        self.stock_code = ""  # 股票代码
        self.stock_name = ""  # 股票名称
        self.long_margin_ratio = 0.0  # 融资保证金比例
        self.short_margin_ratio = 0.0  # 融券保证金比例
        self.max_long_ratio = 0.0  # 最大融资比例
        self.max_short_ratio = 0.0  # 最大融券比例
```
...
class XtAccountInfo:
    def __init__(self):
        self.account_id = ""  # 账号
        self.account_name = ""  # 账号名称
        self.account_type = 0  # 账号类型
        self.market_list = []  # 交易市场列表
```
...
- 释义
  - 开启主动请求接口的专用线程，以支持在on_stock_order等推送接口中调用同步请求
- 参数
  - enabled - bool 是否开启
- 返回
  - 无
- 备注
  - 开启宽松时序时，查询返回的resp4由专用线程返回，程序正常执行，但此时查到的resp4是push3之后的状态
  - 推荐在on_stock_order等推送回调中使用查询接口的异步版本
...
- 返回
  - 返回委托ID，成功委托后的委托ID为大于0的正整数，如果为-1表示委托失败
- 备注
...
- 示例
  - 股票资金账号1000000365对委托ID为'123456'的委托进行撤单
...
- 参数
  - account - StockAccount 资金账号
  - market - str 市场代码
  - order_sysid - str 委托系统号
- 返回
  - 返回撤单结果，成功返回0，失败返回-1
- 备注
  - 如果失败，则通过撤单失败主推接口返回撤单失败信息
- 示例
  - 股票资金账号1000000365对市场为'SH'，系统号为'123456'的委托进行撤单
...
- 参数
  ...
  - order_id - str 委托ID
- 返回
  - 返回撤单请求序号seq，成功委托后的撤单请求序号为大于0的正整数，如果为-1表示撤单失败
- 备注
  - 如果失败，则通过撤单失败主推接口返回撤单失败信息
...
```python
cancel_order_stock_async_sysid(account, market, order_sysid)
```
...
```python
sync_transaction_from_external(account, trade_data)
```
...
- 参数
  - account - StockAccount 资金账号
  - cancelable_only - bool 是否仅查询可撤委托，默认为False
- 返回
  - list[XtOrder] 委托列表
...
```python
export_data(data_type, start_time, end_time, account)
```
...
```python
query_data(data_type, params, account)
```
...
```python
smt_negotiate_order(account, stock_code, volume, days)
```
...
def on_account_status(self, account_status):
    """
    账号状态信息推送
    :param account_status: XtAccountStatus 账号状态
    """
    pass
```
...
def on_order_error(self, order_error):
    """
    下单失败信息推送
    :param order_error: XtOrderError 下单失败错误对象
    """
    pass
```
...
def on_cancel_error(self, cancel_error):
    """
    撤单失败信息推送
    :param cancel_error: XtCancelError 撤单失败错误对象
    """
    pass
```
...
def on_order_stock_async_response(self, response):
    """
    异步下单回报推送
    :param response: XtOrderResponse 异步下单委托反馈对象
    """
    pass
```
...
def on_cancel_order_stock_async_response(self, response):
    """
    异步撤单回报推送
    :param response: XtCancelOrderResponse 异步撤单委托反馈对象
    """
    pass
```
...
```python
def on_smt_appointment_response(self, response):
    """
    约券相关异步接口的回报推送
    :param response: XtSmtAppointmentResponse 约券相关异步接口的反馈对象
    """
    pass
```